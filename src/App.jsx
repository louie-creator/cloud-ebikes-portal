...
function UsersPage({ users, onAdd, onToggle, onUpdate, onDelete }) {
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.card}><div style={S.cardTitle}>All Users ({users.length})</div>{users.map(u => (<div key={u.id} style={{ borderBottom: '1px solid var(--border)', padding: '12px 0', opacity: u.active ? 1 : 0.5 }}>{editing?.id === u.id ? (<div style={{ display: 'grid', gap: 10 }}><div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 10 }}><div><div style={{ fontSize: 11, color: 'var(--text2)', fontFamily: 'var(--mono)', marginBottom: 4 }}>NAME</div><input value={editing.name} onChange={e => setEditing({ ...editing, name: e.target.value })} style={S.input} autoFocus /></div><div><div style={{ fontSize: 11, color: 'var(--text2)', fontFamily: 'var(--mono)', marginBottom: 4 }}>CODE</div><input value={editing.code} onChange={e => setEditing({ ...editing, code: e.target.value.replace(/\D/g, '').slice(0, 4) })} style={S.input} maxLength={4} /></div><div><div style={{ fontSize: 11, color: 'var(--text2)', fontFamily: 'var(--mono)', marginBottom: 4 }}>ROLE</div><select value={editing.role} onChange={e => setEditing({ ...editing, role: e.target.value })} style={S.select}><option value="staff">Staff</option><option value="mechanic">Mechanic</option><option value="manager">Manager</option><option value="owner">Owner</option></select></div></div>{editErr && <div style={{ fontSize: 12, color: 'var(--red)' }}>{editErr}</div>}<div style={{ display: 'flex', gap: 8 }}><button onClick={saveEdit} style={{ ...S.btn, ...S.btnP, ...S.btnSm }}>Save</button><button onClick={() => { setEditing(null); setEditErr('') }} style={{ ...S.btn, ...S.btnSm }}>Cancel</button></div></div>) : (<div style={{ display: 'flex', alignItems: 'center', gap: 12 }}><div style={S.avatar(ROLE_COLOR[u.role])}>{u.name[0]}</div><div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 500 }}>{u.name}</div><div style={{ display: 'flex', gap: 8, marginTop: 4, alignItems: 'center', flexWrap: 'wrap' }}><span style={S.badge(ROLE_COLOR[u.role])}>{ROLES[u.role]}</span><span style={{ fontSize: 11, color: 'var(--text3)', fontFamily: 'var(--mono)' }}>Code: {u.code}</span>{!u.active && <span style={S.badge('#ef4444')}>Inactive</span>}</div></div><button onClick={() => setEditing({ id: u.id, name: u.name, code: u.code, role: u.role })} style={{ ...S.btn, ...S.btnSm }}>✏️ Edit</button><button onClick={() => onToggle(u)} style={{ ...S.btn, ...S.btnSm }}>{u.active ? 'Deactivate' : 'Activate'}</button><button onClick={() => onDelete(u.id)} style={{ ...S.btn, ...S.btnD, ...S.btnSm }}>Delete</button></div>)}</div>))}</div>
    </div>
  )
}
        ...
        <>
          <div style={{ ...S.card, marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              <div style={S.cardTitle}>Build Progress</div>
