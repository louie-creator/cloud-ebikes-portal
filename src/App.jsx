...
const PAGE_CONTEXT = {
  shipments: `You are Cloud, an AI assistant built into the Cloud Ebikes staff portal. You are on the Incoming Shipments page. Your primary job is to read supplier invoices and log shipments. When the user uploads a PDF or describes a shipment, extract: supplier, all line items with quantities, ship date, invoice number, and tracking numbers. Then output the data in a <SAVE_SHIPMENT> block so the portal can save it automatically. Always confirm what you saved in plain language.`,
  home: `You are Cloud, an AI assistant built into the Cloud Ebikes staff portal. Help staff with anything — repair questions, product knowledge, store policies, or general questions. Be concise and practical.`,
  workshop: `You are Cloud, an AI assistant for Cloud Ebikes. You are on the Workshop Guides page. Help mechanics with repair questions, troubleshooting, or technical guidance for ebikes and regular bikes.`,
  salesguide: `You are Cloud, an AI assistant for Cloud Ebikes. You are on the Sales Guides page. Help staff answer customer questions about ebikes — brakes, motors, cargo bikes, range, battery life, and objection handling.`,
  'warranty-submit': `You are Cloud, an AI assistant for Cloud Ebikes. Help staff understand what information to collect for warranty claims and how to describe problems clearly.`,
  'warranty-claims': `You are Cloud, an AI assistant for Cloud Ebikes. Help managers process warranty claims, draft supplier follow-up emails, or look up warranty procedures for specific brands.`,
}
...
function FloatingChat({ page, user, onShipmentSaved }) {
  ...
  useEffect(() => {
    if (open && chatMessages.length === 0) {
      const greeting = isShipments
        ? "Hi, I'm Cloud! On this page I can read invoice PDFs and log shipments automatically. Upload a PDF or describe what arrived."
        : "Hi, I'm Cloud! How can I help you today?"
      setChatMessages([{ role: 'assistant', content: greeting }])
    }
  }, [open])

  useEffect(() => {
    if (open) { setChatMessages([]); setInput('') }
  }, [page])
  ...
  const loadPDF = async (file) => {
    setPdfLoading(true)
    setChatMessages(m => [...m, { role: 'user', content: `📄 Uploaded: ${file.name}` }])
    try {
      if (!window.pdfjsLib) {
        await new Promise((resolve, reject) => {
          const s = document.createElement('script')
      ...
      setPdfLoading(false)
      if (!text.trim()) {
        setChatMessages(m => [...m, { role: 'assistant', content: "I couldn't read text from that PDF. Try describing the shipment instead." }])
        return
      }
      await send(`PDF uploaded: ${file.name}. Extracted text:\n\n${text.slice(0, 3000)}`, true)
    } catch (e) {
      setPdfLoading(false)
      setChatMessages(m => [...m, { role: 'assistant', content: "Could not read that PDF. Try a different file or describe the shipment." }])
    }
  }

  const send = async (overrideMsg, skipUserBubble) => {
    const userMsg = overrideMsg || input.trim()
    if (!userMsg || loading) return
    if (!skipUserBubble) setChatMessages(m => [...m, { role: 'user', content: userMsg }])
    setInput(''); setLoading(true)

    const history = chatMessages.slice(-8).map(m => `${m.role === 'user' ? 'User' : 'Cloud'}: ${m.content}`).join('\n')
    const systemNote = isShipments ? `\n\nWhen you have enough shipment info, output it in this exact format:\n<SAVE_SHIPMENT>\n{"supplier":"...","items":[{"name":"...","qty":1}],"expected_date":"YYYY-MM-DD or null","notes":"Invoice #... Tracking: ..."}\n</SAVE_SHIPMENT>` : ''
    ...
    try {
      const res = await fetch(WORKER_URL, {
        ...
        body: JSON.stringify({
          action: 'claude',
          prompt: `${context}${systemNote}\n\nConversation so far:\n${history}\nUser: ${userMsg}\nCloud:`
        })
      })
      const d = await res.json()
      const reply = (d.text || 'Sorry, I could not get a response.').trim()
      ...
      if (saveMatch && isShipments) {
        try {
          ...
          const item = { id: uid(), ...parsed, status: 'In Transit', added_by: user.name, created_at: nowISO() }
          await sb.from('shipments').insert(item)
          const cleanReply = reply.replace(/<SAVE_SHIPMENT>[\s\S]*?<\/SAVE_SHIPMENT>/, '').trim()
          setChatMessages(m => [...m, { role: 'assistant', content: (cleanReply || 'Done!') + '\n\n✅ Shipment saved — it will appear in the list above.' }])
          if (onShipmentSaved) onShipmentSaved()
        } catch (e) {
          setChatMessages(m => [...m, { role: 'assistant', content: reply }])
        }
      ...
      setChatMessages(m => [...m, { role: 'assistant', content: 'Error reaching Cloud AI. Try again.' }])
    }
    setLoading(false)
  }
  ...
  return (
    <>
      <button onClick={() => setOpen(o => !o)} style={{ position: 'fixed', bottom: 24, right: 24, width: 54, height: 54, borderRadius: '50%', background: open ? 'var(--bg3)' : 'var(--accent)', border: '1px solid var(--border2)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, boxShadow: '0 4px 24px rgba(59,130,246,0.35)', zIndex: 1000, transition: 'background 0.2s' }}>
        {open ? '✕' : '⚡'}
      </button>
      {open && (
        <div style={{ position: 'fixed', bottom: 90, right: 24, width: 360, height: 500, background: 'var(--bg2)', border: '1px solid var(--border2)', borderRadius: 16, display: 'flex', flexDirection: 'column', zIndex: 1000, boxShadow: '0 8px 40px rgba(0,0,0,0.5)' }}>
          <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, fontWeight: 700, color: 'white' }}>⚡</div>
            <div>
              <div style={{ fontSize: 14, fontWeight: 700 }}>Cloud</div>
              <div style={{ fontSize: 11, color: 'var(--text2)' }}>{isShipments ? 'Shipments mode — upload an invoice PDF' : 'AI Assistant · Cloud Ebikes'}</div>
            </div>
          </div>
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
            {chatMessages.map((m, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
                <div style={{ maxWidth: '88%', padding: '9px 13px', borderRadius: m.role === 'user' ? '14px 14px 3px 14px' : '14px 14px 14px 3px', background: m.role === 'user' ? 'var(--accent)' : 'var(--bg3)', border: m.role === 'assistant' ? '1px solid var(--border)' : 'none', fontSize: 13, lineHeight: 1.65, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                  {m.content}
                </div>
              </div>
            ))}
          ...
          <div style={{ padding: '10px 12px', borderTop: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 8 }}>
            {isShipments && (
              <>
                <input ref={fileRef} type="file" accept="application/pdf" onChange={e => { if (e.target.files[0]) { loadPDF(e.target.files[0]); e.target.value = '' } }} style={{ display: 'none' }} />
                <button onClick={() => fileRef.current?.click()} disabled={pdfLoading || loading} style={{ ...S.btn, ...S.btnSm, width: '100%', justifyContent: 'center', fontSize: 12, opacity: (pdfLoading || loading) ? 0.6 : 1 }}>
                  📄 Upload Invoice PDF
                </button>
              </>
            )}
            <div style={{ display: 'flex', gap: 8 }}>
              <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && !loading && !pdfLoading && send()} placeholder={isShipments ? 'Ask about a shipment...' : 'Ask Cloud anything...'} style={{ ...S.input, flex: 1, fontSize: 12 }} disabled={loading || pdfLoading} />
              <button onClick={() => send()} disabled={loading || pdfLoading || !input.trim()} style={{ ...S.btn, ...S.btnP, ...S.btnSm, opacity: (loading || !input.trim()) ? 0.5 : 1 }}>↑</button>
            </div>
          </div>
        </div>
